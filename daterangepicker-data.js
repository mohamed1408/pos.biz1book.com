...
$(function () {
	...
	function cb(start, end) {
		...
		var totdate = $('#reportrange')[0].innerText.split(' - ')
		console.log(moment(totdate[0]).format('YYYY-MM-DD'))
		var settings = {
			"async": true,
			"crossDomain": true,
			"url": `https://biz1pos.azurewebsites.net/api/Dashboard/Post/?fromDate=${moment(totdate[0]).format('YYYY-MM-DD')}&toDate=${moment(totdate[1]).format('YYYY-MM-DD')}&compId=${companyid}&storeId=${storeid}`,
			"method": "GET",
			"headers": {
		...
		$.ajax(settings).done(function (response) {
			try {
				...
				var storeTransactions = response["StoreTransactions"]
				...
				if (response.hasOwnProperty('TotalSales')) {
					response.TotalSales.forEach(element => {
						...
						cardinnerhtml = cardinnerhtml +
							`<tr>
								...
								<td>&#8377;${nfObject.format(storeTransactions.filter(x => x.Id == element.StoreId)[0].Amount)}qwerty</td>
					...
					console.log(response.Payments, response.Payments[0].Payments)
